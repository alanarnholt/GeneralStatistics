...
model <- train(homeless ~ i1 + sex + g1b + pss_fr + dayslink + sexrisk, data = HELP, method = "glm",
               trControl = myControl)
...
#####
# Train glm with custom trainControl: model
model <- train(homeless ~ ., data = HELP, method = "glm",
               trControl = myControl)
...
model <- train(
  ...
  tuneGrid = expand.grid(alpha  = 0:1, lambda = seq(0.0001, 10, length = 20)),
  method = "glmnet",
  metric = "ROC",
  trControl = myControl,
